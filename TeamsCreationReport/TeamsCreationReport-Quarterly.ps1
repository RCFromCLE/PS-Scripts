...
$SmtpPort = '25'
$EmailRecipient = "o365support@company.com" # &lt;- Update this with the real address
...
$EmailRecipient = "rudy.corradetti@company.com" # &lt;- Update this with the real address
# Find records for team creation in the Office 365 audit log
Write-Host "Looking for Team Creation Audit Records, Quarter Starting on: $when..."
...
# Construct the message parameters and send it off...
 $MsgParam = @{
     To = $EmailRecipient
     From = $MsgFrom
     Subject = "Quarterly Teams Creation Report"
     Body = $HtmlMsg
     SmtpServer = $SmtpServer
     Port = $SmtpPort
     Credential = $anon_credentials}
Send-MailMessage @msgParam -BodyAsHTML ; Write-Host "Quarter Starting on: $when" $EmailRecipient 